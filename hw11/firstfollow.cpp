    ...
    while (changed) {
        ...
        for (const auto &pair : grammar) {
            for (const auto &rightPart : pair.second) {
                ...
                std::set<std::string> diffResult;
                std::set_difference(f.begin(), f.end(), set.begin(), set.end(),
                                    std::inserter(diffResult, diffResult.begin()));
                if (!diffResult.empty()) {
...
bool Grammar::isNonTerminal(const std::string &word) {
    if (word == EPSILON) return false;
    return nonTerminals.find(word) != terminals.end();
}
